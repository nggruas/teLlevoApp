...
getUserProfile() {
  ...
  getTripDetails(tripId: string): Observable<Trip> {
    ...
    // Método para obtener el historial de viajes por ID de usuario
    getHistorialViajesByUserId(userId: string): Observable<Trip[]> {
      const tripsRef = collection(getFirestore(), 'trips');
